...
func GetSecrets(ctx *gin.Context) {
	...
	results := make([]*model.TSecret, 0)
	for {
		namespaces, err := client.CoreV1().Secrets(namespace).List(context.Background(),
		...
		for _, v := range namespaces.Items {
			results = append(results, &model.TSecret{
				Id:         len(results) + 1,
				Name:       v.Name,
				Uid:        string(v.UID),
