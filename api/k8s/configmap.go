...
func GetConfigmaps(ctx *gin.Context) {
	...
	results := make([]*model.TConfigmap, 0)
	for {
		namespaces, err := client.CoreV1().ConfigMaps(namespace).List(context.Background(),
		...
		for _, v := range namespaces.Items {
			results = append(results, &model.TConfigmap{
				Id:         len(results) + 1,
				Name:       v.Name,
				Uid:        string(v.UID),
